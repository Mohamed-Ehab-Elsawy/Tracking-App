...
import 'package:tracking_app/features/orders/presentation/order_history/view/widgets/orders_number_card.dart';
...
class _OrderHistoryViewState extends State<OrderHistoryView> {
  ...
  void initState() {
    ...
    checkoutCubit.orderHistoryUiEvent.listen((event) {
      if (event is NavigateToOrderDetails) {
        ...
        Navigator.pushNamed(context, AppRoutes.orderDetails, arguments: event.order);
      }
    });
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: Text("Orders").tr(), centerTitle: true),
      body: SingleChildScrollView(
        child: BlocBuilder<OrderHistoryCubit, OrderHistoryStates>(
          builder: (context, state) {
            final orders = state.ordersList;

            if (orders == null || orders.requestState == RequestState.loading) {
              return const Center(child: CircularProgressIndicator());
            }

            if (orders.requestState == RequestState.error) {
              return Center(child: Text(orders.errorMessage ?? "something went wrong".tr()));
            }
            ...
            return Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  SizedBox(height: 15),
                  Text(" Recent orders : ", style: context.textStyles.medium20).tr(),
                  SizedBox(height: 20),
                  ListView.builder(
                    shrinkWrap: true,
                    ...
                    itemBuilder: (context, index) {
                      ...
                      return InkWell(
                        ...
                        child: OrderCard(
                          ...
                          shopAddress:
                              order?.store?.address ?? "un known address".tr(),
                          shopName: order?.store?.name ?? "un known name".tr(),
                          userImagePath:
                          ...
                          userAddress:
                              ...
                              "un known address".tr(),
                          userName:
                              order?.order?.user?.firstName ?? "un known name".tr(),
                        ),
                      );
                    },
                  ),
